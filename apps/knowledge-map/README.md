...
A local 3D explorer with no install or build dependencies for Markdown and text files. Real world-space coordinates rotate around two axes and use perspective projection, depth ordering, and lighting, rendered with the browser's Canvas API. No external services, fonts, or network calls are needed.
...
Sphere colors identify sources. Straight lines represent resolved explicit Markdown or wiki links; the faint curved globe and orbit guides are decorative. Counts always reflect indexed files, including empty categories. A small knowledge base produces a spacious map rather than invented nodes. The note reader defaults to **Formatted** text. Use the **Formatted / Markdown** toggle above the content to switch to the exact source; your preference is remembered across notes and reloads. Formatted view supports headings, lists, tasks, tables, quotes, and code. Resolved relative note links open in the reader; external links open in a new tab. Embedded HTML and image descriptions remain text, without executing HTML or fetching images. Plain `.txt` files retain their layout.

Markdown is tokenized by the bundled Marked 16.4.2 (`vendor/marked.js`, MIT license in `vendor/marked.LICENSE.md`) and rendered using DOM elements and text nodes in `markdown.js`; note HTML is never inserted into the page. The bundled parser works offline. See [Marked documentation](https://marked.js.org/) when updating it.
